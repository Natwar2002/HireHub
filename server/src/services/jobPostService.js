...
export const updateJobPost = async (id, jobId, jobDetailsData) => {
  try {
    ...
    const updatedJobPost = await jobPostRepository.update(
      jobId,
      jobDetailsData
    );
...
export const getJobPostByHR = async (userId) => {
  try {
    const page = 1;
    const limit = 20;
    const jobposts = await jobPostRepository.getJobsPostedByHR(
      page,
      limit,
      userId
    );
...
export const getJobsForDashboard = async (recruiterId) => {
  try {
    const page = 1;
    const limit = 20;
    const jobs = await jobPostRepository.getDashboardData(page, limit, recruiterId);
    return jobs;
  } catch (error) {
